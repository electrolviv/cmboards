...
VHIncEncoder::VHIncEncoder(const cfg *pcfg)
    : _value(0), _dir(false), _encPosOld(0), _ptrconfig((void *)pcfg),
      _changed(false) {}

void VHIncEncoder::begin() {

  GPIO_InitTypeDef GPIO_InitStruct = {0};
  cfg *pcfg = static_cast<cfg *>(_ptrconfig);
...
void VHIncEncoder::calcpos(uint8_t newpos) {

  // Walk through table
  uint8_t tblbyte = enctbl[_encPosOld];
  uint8_t tblval = (tblbyte >> (newpos << 1));
...
bool VHIncEncoder::isChanged() { return _changed; }

void VHIncEncoder::resetevt() { _changed = false; }

int32_t VHIncEncoder::readValue() { return _value; }

bool VHIncEncoder::readDirection() { return _dir; }
